...
object Util {
  ...
  def unableToConvert(src: Any, dest: Class[_]) =
    throw new Exception(s"I don't know how to convert $src of type ${src.getClass.getCanonicalName} to ${dest.getCanonicalName}")

  def set(ctr: Int, st: PreparedStatement, data: Any, dataType: DataType)(implicit ignoreUnsigned: Boolean = true): Unit = {
    def signException(x: Number, d: DataType) =
      if (!ignoreUnsigned) throw new SQLException("unsigned constraint violation: [" + x + "] for [" + d + "]")
    def rangeException(n: Number, x: Any, d: DataType) =
      throw new SQLException("range constraint violation: [" + x + "] for [" + d + "] (limit: " + n + ")")
...
//        println("[DB] (Set) Ignoring data (ctr = "+ctr+", data: "+data + ", type: "+ dataType)
  ...
  def canDoComparison(col: Col, op: Op) =
    (col.colType, op) match {
  ...
  def incrementItWithResult(a: Array[Any], incrVals: Array[Any]): Array[(Any, Any)] =
    (a zip incrVals).map {
