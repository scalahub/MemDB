...
case class Table(tableName: String, tableCols: Cols, priKey: Cols) {
  ...
  def assertColExists(c: Col) =
    c.compositeCols.foreach { col =>
  ...
  protected[db] def createSQLString =
  ...
  def insertSQLString = {
    val (actualCols, qMark) = tableCols
      .collect {
...
object Table {
  ...
  private def getOrderStr(orderings: Orderings) = {
    ...
    " ORDER BY " + orderings
      .map { x =>
        x.col.colSQLString + getOrdering(x.isDescending)
      }
      .reduceLeft(_ + "," + _)
  }
  ...
  private def whereString(wheres: Wheres, str: String, joinOp: String) =
    if (wheres.size == 0) ""
    else {
      " " + str + " " + wheres.map { _.whereSQLString }.reduceLeft(_ + " " + joinOp + " " + _)
    }

  private def getSelectString(cols: Cols) =
    if (cols.size == 0) "*" else cols.map(col => col.colSQLString + " AS " + col.alias).reduceLeft(_ + "," + _)
