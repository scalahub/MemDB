package org.memdb.db.core.DataStructures

import org.memdb.db.core.Table
import org.memdb.db.core.Util
...
case class Where(col: Col, op: Op, data: Any) {
  def checkValidOp =
    if (!Util.canDoComparison(col, op))
      throw new Exception(s"invalid op $op for col $col of type ${col.colType} in table ${col.optTable.getOrElse("none")}")
