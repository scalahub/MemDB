package org.memdb.db

import java.sql.{Connection, PreparedStatement, ResultSet, Statement}

import org.memdb.db.config.DBConfig
import org.memdb.db.core.DataStructures.{Col, Where, _}
import org.memdb.db.core.Table
import org.memdb.db.core.Util._
...
private[db] abstract class DBManagerDML(table: Table, dbConfig: DBConfig) {
  ...
  private[db] def insertArray(data: Array[Any])(implicit optConn: OptConn = None) = {
    ...
    val returnAutoIncrementIndex = if (actualDataCols.length == tableCols.length) false else true
    if (data.length != actualDataCols.length)
      throw new Exception(
        s"Schema mismatch for table $table. Expected: ${actualDataCols.length}, found: ${data.length}. [AutoIncrment columns should not have data]"
      )
  ...
  private[db] def updateCols(wheres: Wheres, updates: Updates[Any])(implicit optConn: Option[java.sql.Connection] = None): Int = {
    ...
    usingOpt(optConn, getConnection) { conn =>
      using(conn.prepareStatement(updateSQLString(cWheres, cUpdates))) { st =>
        {
      ...
      lock.synchronized {
        ...
        try {
          ...
          val resCols = aftersAndBefores.size match {
            case 0 =>
              throw new Exception(s"Zero rows matched in table $table while incrementing cols ${incrCols.map(_.name).reduceLeft(_ + "," + _)}")
            case 1 =>
              val afterAndBefore    = aftersAndBefores.head
  ...
  private[db] def selectInto(db: DBManager, cWheres: Wheres, cCols: Cols)(implicit
      cOrderings: Orderings = Array(),
      limit: Int = 0,
      offset: Long = 0,
  ...
  private def selectResultSet[T](cWheres: Wheres, cCols: Cols, func: ResultSet => T)(implicit
      cOrderings: Orderings = Array(),
      limit: Int = 0,
      offset: Long = 0,
      optConn: OptConn = None
  ...
  private def selectAnyResultSet[T](
    ...
    usingOpt(optConn, getConnection) { conn =>
      using(conn.prepareStatement(getSQLString(cCols, cWheres, cOrderings, limit, offset))) { st =>
        {
          setData(
            getSelectData(cCols: Cols, cWheres: Wheres, cOrderings: Orderings),
            0,
            st
          )
          doSQLQuery(st)
  ...
  private[db] def selectCols[T](wheres: Wheres, cols: Cols, func: Array[Any] => T)(implicit
      orderings: Orderings = Array(),
      limit: Int = 0,
      offset: Long = 0,
      optConn: OptConn = None
  ): List[T] = {
  ...
  private[db] def selectRS[T](wheres: Wheres, cols: Cols, func: ResultSet => T)(implicit
      orderings: Orderings = Array(),
      limit: Int = 0,
      offset: Long = 0,
      optConn: OptConn = None
