...
import org.memdb.db.{DBManager => DBM}
...
object ScalaDB {
  ...
  case class Sel[T](qry: DBSelect[T]) {
    ...
    def orderBy(orderings: Orderings) =
      Sel(db, origCols, origWheres, origMax, origOffset, origOrderings ++ (orderings.filterNot(origOrderings.contains)), origToT)
