...
import org.memdb.db.core.{Table, _}
...
class DBManager(private[db] val table: Table)(implicit private[db] val dbConfig: DBConfig) extends DBManagerDML(table: Table, dbConfig: DBConfig) {
  ...
  def createTableIfNotExists =
    using(getConnection) { conn =>
      using(conn.prepareStatement(createSQLString))(_.executeUpdate)
    }
